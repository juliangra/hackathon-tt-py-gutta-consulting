...
def _sm_mkt(cfg: TranslationConfig, i: str, lines: list[str]) -> None:
    ...
    lines.append(f"{i}    if _bs: {v('unitPriceAtEndDate')} = D(str(_bs[-1].get('unitPrice', 0)))")
    lines.append(f"{i}if not {v('unitPriceAtEndDate')} or {v('unitPriceAtEndDate')} == D(0):")
...
def _sm_orders(cfg: TranslationConfig, i: str, lines: list[str]) -> None:
    ...
    lines.append(f"{i}    unitPrice={v('unitPriceAtStartDate')}, fee=D(0), itemType='start', {upm}={v('unitPriceAtStartDate')}))")
    lines.append(f"{i}orders.append(dict(date=end_str, type={first_pos!r}, quantity=D(0),")
    lines.append(f"{i}    unitPrice={v('unitPriceAtEndDate')}, fee=D(0), itemType='end', {upm}={v('unitPriceAtEndDate')}))")
    lines.append("")
...
def _sm_fill(cfg: TranslationConfig, i: str, lines: list[str]) -> None:
    ...
    lines.append(f"{i}            if '{upm}' not in o: o['{upm}'] = _lup or o['unitPrice']")
    lines.append(f"{i}    else:")
    lines.append(f"{i}        _up = _lup or D(0)")
    lines.append(f"{i}        s = dict(date=ds, type={first_pos!r}, quantity=D(0), unitPrice=_up, fee=D(0), itemType=None, {upm}=_up)")
...
def _sm_loop(cfg: TranslationConfig, i: str, lines: list[str]) -> None:
    ...
    for atype, factor in cfg.activity_factors.items():
        if factor == 0 and atype != "FEE":
            ...
            lines.append(f"{ii}{cond} otype == {atype!r}: {target} += order['quantity'] * order['unitPrice']")
    ...
    lines.append(f"{ii}_up = order['unitPrice'] if otype in {pos!r} else order.get('{upm}', order['unitPrice'])")
...
def _perf_chart(cfg: TranslationConfig, i: str, lines: list[str]) -> None:
    ...
    lines.append(f"{i}        netPerformance=0, netPerformanceInPercentage=0,")
    ...
    lines.append(f"{i}        totalInvestment=float(_inv), netPerformance=float(_npv),")
    lines.append(f"{i}        netPerformanceInPercentage=float(_npv / _tw) if _tw > 0 else 0.0,")
    lines.append(f"{i}        netPerformanceInPercentageWithCurrencyEffect=float(_npv / _tw) if _tw > 0 else 0.0,")
...
def _perf_return(cfg: TranslationConfig, i: str, lines: list[str]) -> None:
    ...
    lines.append(f"{i}    netPerformance=float(_tnp), netPerformancePercentage=float(_np),")
    lines.append(f"{i}    netPerformancePercentageWithCurrencyEffect=float(_np),")
    lines.append(f"{i}    netPerformanceWithCurrencyEffect=float(_tnp),")
...
def _emit_hold(cfg: TranslationConfig, lines: list[str]) -> None:
    ...
    lines.append(f"{i}        marketPrice=m.get('mp', 0.0), netPerformance=float(m['_tnp']),")
    lines.append(f"{i}        netPerformancePercent=float(m['_npp']),")
    lines.append(f"{i}        netPerformancePercentage=float(m['_npp']),")
