...
from tt.transforms.stdlib import try_date_fns, try_lodash, try_array_method
...
def _translate_method_call(func_node: Node, args: list, cfg: TranslationConfig) -> pyast.expr:
    ...
    if prop_node:
            ...
            result = try_array_method(method_name, obj_node, args, cfg)
...
def _translate_func_call(func_node, args: list, cfg: TranslationConfig) -> pyast.expr:
    ...
    result = try_date_fns(func_name, args)
    if result is not None:
        return result
    result = try_lodash(func_name, args, func_node, cfg)
    if result is not None:
        return result
